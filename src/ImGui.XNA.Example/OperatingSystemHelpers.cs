...
public unsafe static class OperatingSystemHelpers
{
    ...
    public static bool HasLinuxClipboardDependency()
    {
        ...
        {
            Marshal.Prelink(
                typeof(OperatingSystemHelpers).GetMethod(
                    nameof(OperatingSystemHelpers.SDL_GetClipboardText),
                    BindingFlags.NonPublic | BindingFlags.Static
                )!
            );
            Marshal.Prelink(
                typeof(OperatingSystemHelpers).GetMethod(
                    nameof(OperatingSystemHelpers.SDL_SetClipboardText),
                    BindingFlags.NonPublic | BindingFlags.Static
                )!
            );
            return true;
    ...
    public static readonly IntPtr? GetFnPtr = typeof(OperatingSystemHelpers).
                                              GetMethod(nameof(Get), BindingFlags.NonPublic | BindingFlags.Static)?.
    ...
    private static bool SetTextForOsx(string text)
    {
        ...
        try
        {
            ...
            var str = objc_msgSend(
                objc_msgSend(nsString, sel_registerName("alloc")),
                sel_registerName("initWithUTF8String:"),
                text
            );
            var dataType = objc_msgSend(
                objc_msgSend(nsString, sel_registerName("alloc")),
                sel_registerName("initWithUTF8String:"),
    ...
    private static string? GetTextForOsx()
    {
        try
        {
            ...
            var nsStringPboardType = objc_msgSend(
                objc_msgSend(nsString, sel_registerName("alloc")),
                sel_registerName("initWithUTF8String:"),
                "NSStringPboardType"
            );
